...
### 🟢 Ready (Next Up)
- **Fix `vpn-watchdog.sh` (lives only on the NAS at `/usr/local/bin/`, not version-controlled)** — three defects found 2026-09-08:
  1. `healthy()` hardcodes `ip route | head -1 | grep -q tun0`, so a perfectly healthy tunnel on `tun1` reads as UNHEALTHY and triggers endless reconnects.
  2. Reconnect spawns a second `openvpn` daemon without reaping the first, which is how the NAS ended up with two daemons and two `tun` devices fighting over the default route.
  3. Its "NAS left on direct LAN internet (un-VPN'd but working)" fallback never verifies the default route actually fell back to the LAN gateway — so it logs that line while the box is in fact black-holed. It must explicitly delete stale `tun` default routes and restore `default via 192.168.0.1 dev eth1`.
  Also worth adding: alert after N consecutive failures. This ran silently for 5 days. Should be committed into `claude-synology` so it stops being an untracked one-off.

### 📋 Backlog
[Empty]

### 🔴 Blocked
- **NAS outbound network is black-holed — needs Charles to run the recovery commands (2026-09-08)** — Root cause of the mode-switch failure emails. Two OpenVPN daemons are alive on the NAS (PID 19783 from Aug 13 owning `tun0`, PID 11788 from Sep 3 owning `tun1`); the dead Sep 3 tunnel still holds the default route (`default via 10.100.0.1 dev tun1`), so all NAS outbound traffic — DNS included — is dropped. Broken since 2026-09-03 04:22. `vpn-watchdog.sh` has logged 724 consecutive UNHEALTHY cycles and cannot self-heal, because its escalation path needs the internet it has black-holed. Attempted `synovpnc kill_client` + orphan-daemon cleanup from this session; **blocked by the Claude Code permission classifier** (NAS network mutation). Recovery commands handed to Charles.
